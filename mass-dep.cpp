...
double R2 = unit::Rj;
void mass_job(double mj, double fv1, double fv2, size_t tot_num) {
    double a2 = 10;
    double a1 = a2 * fv2;
    ...
    double r_start = 30 * a2;

    char filename[100] = {0};

    sprintf(filename, "new-results/mass-%.3f-%.3f-%.3f.txt", mj / 1e-3, fv1, fv2);
    ...
    for (size_t k = 0; k < tot_num; ++k) {
        ...
        double b = sqrt(random::Uniform(0, b_max * b_max));
        ...
        args.add_stop_point_operation([&](auto& ptc, auto h) {
            ...
            double v_ej = 0;
            if (status == -1) {
                file << status << ' ' << b_max << ' ' << b << ' ' << 0 << ' ' << 0 << ' ' << v_ej / v_orb << ' '
                     << v_inf / v_orb << ' ' << r_ej << '\n';
                return;
            }
            ...
            if (a10 > 0 && a20 > 0) {
                ...
                auto vcm = ptc.mass(1) * ptc.vel(1) + ptc.mass(2) * ptc.vel(2);
                auto rcm = ptc.mass(1) * ptc.pos(1) + ptc.mass(2) * ptc.pos(2);
                r_ej = norm(rcm - ptc.pos(0));
                v_ej = norm(vcm - ptc.vel(0));

                v_inf = sqrt(v_ej * v_ej - 2 * ptc.mass(0) / r_ej);
            }
            file << status << ' ' << b_max << ' ' << b << ' ' << a12 / a2 << ' ' << e12 << ' ' << v_ej / v_orb << ' '
                 << v_inf / v_orb << ' ' << r_ej << '\n';
        });
...
int main() {
    ...
    double mj_max = 1e-2;
    int mass_point = 25;
    double dm = (mj_max - mj_min) / mass_point;
    size_t tot_run = 1000000;
    for (double mj = mj_min; mj < mj_max; mj += dm) {
        executor.silent_async(mass_job, mj, 0.2, 0.7, tot_run);
    }

    for (double mj = mj_min; mj < mj_max; mj += dm) {
        executor.silent_async(mass_job, mj, 0.5, 0.7, tot_run);
    }

    for (double mj = mj_min; mj < mj_max; mj += dm) {
        executor.silent_async(mass_job, mj, 1, 0.7, tot_run);
    }

    for (double mj = mj_min; mj < mj_max; mj += dm) {
        executor.silent_async(mass_job, mj, 10, 0.7, tot_run);
    }
